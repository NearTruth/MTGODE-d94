...
class STBlock(nn.Module):  # what does ST stand for??  spatial-temporal??
    ...
    def forward(self, x):
        ...
        self.new_dilation *= self.dilation_factor  # what is the use of the dilation
        self.intermediate_seq_len = x.size(3)

        x = F.dropout(x, self.dropout, training=self.training)

        x = self.gconv_1(x, self.graph) + self.gconv_2(x, self.graph.transpose(1, 0))  # why do we need 2 gconv
...
class MTGODE(nn.Module):  # main model
    ...
    def forward(self, input, idx=None):
        ...
        if self.seq_length < self.receptive_field:  # pad with 0
